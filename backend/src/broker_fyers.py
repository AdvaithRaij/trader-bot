...
class FyersBroker:
    ...
    async def _update_account_info(self):
        ...
        try:
            ...
            if funds['s'] == 'ok':
                ...
                # id=1: Total Balance, id=2: Utilized Amount, id=10: Available Balance
                fund_dict = {item['id']: item for item in funds['fund_limit']}

                total_balance = fund_dict.get(1, {}).get('equityAmount', 0)
                utilized_amount = fund_dict.get(2, {}).get('equityAmount', 0)
                available_balance = fund_dict.get(10, {}).get('equityAmount', 0)

                self._account_info = {
                    'available_balance': available_balance,
                    'used_margin': utilized_amount,
    ...
    async def get_ltp(self, symbol: str) -> float:
        ...
        try:
            ...
            # Format symbol for Fyers API if not already formatted
            if ':' not in symbol:
            ...
            if quotes.get('s') == 'ok' and quotes.get('d'):
                ltp = quotes['d'][0]['v']['lp']
                logger.debug(f"LTP for {fyers_symbol}: ₹{ltp:.2f}")
                return ltp
            else:
    ...
    async def get_historical_data(self, symbol: str, days: int = 30) -> List[Dict]:
        ...
        try:
            if config.MOCK_MODE:
                ...
                for i in range(days):
                    date = dt.now() - timedelta(days=days-i)
            ...
            if response['s'] == 'ok':
                ...
                for candle in candles:
                    historical_data.append({
                        'date': dt.fromtimestamp(candle[0]).strftime('%Y-%m-%d'),
                        'open': candle[1],
                        'high': candle[2],
                        'low': candle[3],
                        'close': candle[4],
