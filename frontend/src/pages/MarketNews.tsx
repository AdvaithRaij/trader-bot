...
import type { NewsItem, NewsResponse } from '../lib/trading-api'
...
export function MarketNews() {
  ...
  // Get all filter options from backend (static lists, not dynamic based on data)
  const sentiments = ['all', ...(newsResponse?.filters.availableSentiments || ['positive', 'negative', 'neutral'])]
  const categories = ['all', ...(newsResponse?.filters.availableCategories || ['market', 'finance', 'economy', 'corporate', 'policy'])]
  const sources = ['all', ...(newsResponse?.filters.availableSources || ['Economic Times', 'Google News', 'MoneyControl'])]

  // Fetch news on component mount and when server-side filters change
  useEffect(() => {
    fetchNews()
  }, [categoryFilter, sourceFilter, currentPage])
  ...
  const fetchNews = async (refresh: boolean = false) => {
    ...
    try {
      ...
      // Transform backend news to match frontend format
      const transformedNews = response.news.map(item => ({
        ...
        confidence: item.aiAnalysis?.relevance || 50,  // Use AI relevance score instead of hardcoded 85
        readTime: 3
      }))

      setNews(transformedNews)
      // Apply only client-side filters (search, sentiment)
      // Source is now server-side
      applyClientFilters(transformedNews)
    } catch (error) {
  ...
  // Apply client-side filters (NOT category or source - those are server-side)
  const applyClientFilters = (newsItems: NewsItem[]) => {
    ...
    // Source filter is now server-side, removed from here
        ...
        {newsResponse && newsResponse.pagination.totalPages > 1 && (
          <div className="mt-8 flex items-center justify-between border-t border-gray-800 pt-6">
            <div className="text-sm text-gray-400">
              Showing {((newsResponse.pagination.currentPage - 1) * newsResponse.pagination.itemsPerPage) + 1} to{' '}
              {Math.min(
                newsResponse.pagination.currentPage * newsResponse.pagination.itemsPerPage,
                newsResponse.pagination.totalItems
              )}{' '}
              of {newsResponse.pagination.totalItems} articles
            </div>

            <div className="flex items-center gap-2">
              <button
                onClick={() => setCurrentPage(prev => Math.max(1, prev - 1))}
                disabled={!newsResponse.pagination.hasPrevious}
                className={cn(
                  "px-4 py-2 rounded-lg font-medium transition-colors",
                  newsResponse.pagination.hasPrevious
              ...
              <div className="flex items-center gap-1">
                {Array.from({ length: Math.min(5, newsResponse.pagination.totalPages) }, (_, i) => {
                  const pageNum = i + 1
                  return (
                    <button
                      key={pageNum}
                ...
                {newsResponse.pagination.totalPages > 5 && (
                  <>
                    <span className="text-gray-500 px-2">...</span>
                    <button
                      onClick={() => setCurrentPage(newsResponse.pagination.totalPages)}
                      className={cn(
                        "w-10 h-10 rounded-lg font-medium transition-colors",
                        currentPage === newsResponse.pagination.totalPages
                          ? "bg-blue-600 text-white"
                          : "bg-gray-800 text-gray-300 hover:bg-gray-700"
                      )}
                    >
                      {newsResponse.pagination.totalPages}
                    </button>
                  </>
                )}
              </div>
              ...
              <button
                onClick={() => setCurrentPage(prev => Math.min(newsResponse.pagination.totalPages, prev + 1))}
                disabled={!newsResponse.pagination.hasNext}
                className={cn(
                  "px-4 py-2 rounded-lg font-medium transition-colors",
                  newsResponse.pagination.hasNext
                    ? "bg-gray-800 text-white hover:bg-gray-700"
                    : "bg-gray-900 text-gray-600 cursor-not-allowed"
                )}
              >
