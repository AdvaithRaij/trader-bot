...
export function TradeLogs() {
  ...
  const statuses = ['all', 'executed', 'pending', 'failed', 'cancelled', 'unknown']
  const sides = ['all', 'buy', 'sell']
  const strategies = ['all', ...Array.from(new Set(trades.map(trade => trade.strategy).filter(Boolean)))]
  ...
  const loadTrades = async () => {
    ...
    try {
      ...
      const response = await apiService.getTradeHistory(days)
  ...
  // Load trades on mount and when date range changes
  useEffect(() => {
    loadTrades()
  }, [dateRange])
  ...
  }, [trades, statusFilter, sideFilter, strategyFilter])
  ...
  const executedTrades = filteredTrades.filter(t => t.status === 'executed' || t.status === 'unknown')
  const totalPnL = executedTrades.reduce((sum, trade) => sum + (trade.pnl || 0), 0)
  const totalFees = executedTrades.reduce((sum, trade) => sum + (trade.fees || 0), 0)
                ...
                >
                  <td className="px-6 py-4">
                    <div>
                      ...
                      {trade.id && (
                        <div className="text-xs text-gray-500">ID: {trade.id.slice(-8)}</div>
                      )}
                    </div>
                  </td>
                  ...
                  <td className="px-6 py-4 text-right">
                    <div className="text-red-400">{formatCurrency(trade.fees || 0)}</div>
                  </td>
                  ...
                  <td className="px-6 py-4">
                    <div className="text-white">{trade.strategy || 'Manual'}</div>
                    {trade.reason && (
